...
Page({
    ...
    goErrorPage: function() {
        wx.redirectTo({
            url: "../error/error?thisBarTitle=商品评价"
        });
    },
    ...
    getData: function(t, a, o, n, r) {
        var s = this;
        e.request({
            url: i.globalRequestUrl + s.data.productDir + "/newComments/newCommentsDetail.json?wareId=" + t + "&offset=" + a + "&num=" + o + "&type=" + n + "&checkParam=LUIPPTP",
            success: r.bind(s),
            complete: function() {
                wx.hideToast();
            },
            fail: function(t) {
                s.goErrorPage(), e.reportErr("assess newCommentsDetail.json: " + t.errMsg);
            }
        });
    },
